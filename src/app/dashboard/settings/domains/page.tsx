...
export default async function DomainsPage() {
  ...
  if (page) {
    ...
    if (sec) {
      ...
      const fieldVal = fieldVals?.find((f: any) => f.field?.name === 'domain_manager_data')

      if (fieldVal) {
        try {
          const raw = fieldVal.published_value_text || fieldVal.value_text
          if (raw) initialDomains = JSON.parse(raw)
        } catch (e) {
          console.error('Error parsing domain manager data', e)
        }
      }
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      <DomainManagerClient initialDomains={initialDomains} />
    </div>
  )
}
